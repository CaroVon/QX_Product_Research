...
| `/{project_id}/assets` | POST | 幻灯片图片暂存 (保存至 outputs/assets/{project_id}/) |
...
> **设计意图**: `/chat` 在 work 模式下先调用 `retrieve_context()` 从项目隔离的 Chroma + BM25 向量库中召回相关文档切片，再注入到 LLM 上下文中。这彻底打通了"本地上传 PDF → 切片入库 → 对话检索"的完整链路。
...
def generate_outline_workflow(self, project_id: str):
    repo = ProjectRepo()
    template_type = repo.get_project_template(project_id)  # 🆕 v0.5 多态模板
    build_knowledge_base(repo.get_project_topic(project_id), project_id)
    outline = generate_outline_task(project_id, template_type)
    repo.save_outline(project_id, outline)
    repo.update_status(project_id, ProjectStatus.WAITING_FOR_OUTLINE)

# Phase 3：逐章撰写 → COMPLETED（v0.5: 不自动生成 PDF，前端 Konva 接管）
@celery_app.task(bind=True, max_retries=3)
def run_draft_sections_workflow(self, project_id: str):
    repo = ProjectRepo()
    template_type = repo.get_project_template(project_id)  # 🆕 v0.5
    search_depth = repo.get_project_search_depth(project_id)  # 🆕 v0.5
    sections = extract_sections(repo.get_outline(project_id))
    for i, section in enumerate(sections):
        content = write_section_task(project_id, section, template_type, search_depth)
...
def generate_outline_task(self, project_id: str, template_type: str = "product") -> str:
    return generate_outline(project_id, template_type)

@celery_app.task(bind=True, max_retries=2, default_retry_delay=60)
def write_section_task(self, project_id: str, section_title: str,
                       template_type: str = "product",
                       search_depth: int = 10) -> str:
    return write_section(project_id, section_title, template_type, search_depth)
```
...
### 9.2 `app/llm/prompts.py` — 🆕 v0.5 PromptFactory 多态模板中枢
...
```python
def generate_outline(project_id: str, template_type: str = "product") -> str:
    """委托 PromptFactory 获取对应模板的 System Prompt，LLM 生成大纲"""
    system_prompt = PromptFactory.get_outline_prompt(template_type)
...
### 9.16 `app/report/section_writer.py` — 章节撰写（含引用 + 模板路由 + v0.4 Prompt 升级）
...
def _write_text_section(topic, section_title, project_id, template_type, search_depth):
    ...
    # v0.5: 通过 PromptFactory.get_section_prompt(template_type) 路由模板
```

**v0.4 Prompt 终极限制**: 规则 5 从建议性改为严厉禁令，三重杜绝 LLM 输出 CSV 伪表格。
**v0.5 多态路由**: `_is_image_section()` 检测「生图/图鉴/概念图」关键词 → 硅基流动图像生成引擎。
...
### 10.5 `pages/EditorPage.tsx` — Canvas 沉浸式编辑器（v0.4 重写）

全屏独立路由，不嵌入 Layout 壳，提供专注的幻灯片编辑体验。v0.4 从 Fabric.js 迁移到 React-Konva + Zustand。
...
**核心功能 (v0.5 完善)**:
- 从 REST API 拉取 blocks → `convertBlocksToKonvaSlides()` → KonvaSlide[] → Zustand store
- 嵌入 `CanvasSlideEditor` 组件（React-Konva 声明式渲染）
- 原生 Konva 离屏 JPEG 导出（`capturePage` → `canvas.toBlob('image/jpeg', 0.92)` → jsPDF）
- AI 助手面板：SSE 流式对话 + "应用到画布" → 直连 Zustand `addElement()`
- v0.5: Undo-Redo 按钮集成 zundo temporal middleware
...
**🆕 `CanvasSlideEditor.tsx`** — React-Konva 幻灯片编辑器（v0.5 完善版，~900 行）：

```tsx
function CanvasSlideEditor({ slides, activeIndex, onActiveIndexChange,
    onSlidesChange, canvasRef, readOnly, showToolbar }: Props) {
  // React-Konva Stage + Layer 架构 (声明式)
  // Transformer 选中框 + 拖拽缩放 (8锚点)
  // 双击文字编辑 (Html textarea 悬浮层)
  // 表格单元格双击编辑 + 行列控制面板 (TableControlPanel)
  // Circle / Line 图形渲染 + 颜色拾取器
  // 排版按钮 (Bold/Italic/Underline/Align) + 字体大小
  // 图层排序 (上移/下移/置顶/置底) + Ctrl+C/V 复制粘贴
  // Undo-Redo (zundo temporal → useTemporalStore)
  // 暴露 editorApi: { addText, addImage, addTable, ... } 给父组件
  // capturePage: 原生 Konva 离屏渲染 → JPEG Blob (Q=0.92) → FileReader → dataURL
  // 预加载图片: new window.Image() + crossOrigin = 'anonymous'
}
```

**核心能力 (v0.5)**:
- 左侧缩略图列表（支持排序/删除/复制页面）
...
**🆕 `useCanvasStore.ts`** — Zustand 原子化 Canvas 状态管理（v0.5 完善版，~228 行）：

```tsx
export interface CanvasElement {
  id: string
  type: 'text' | 'rect' | 'image' | 'table' | 'circle' | 'line'  // 🆕 v0.5
  x: number; y: number; width: number; height: number
  fill?: string; text?: string; src?: string; tableData?: string[][]
  fontWeight?: string; fontStyle?: string; textDecoration?: string  // 🆕 v0.5
  align?: string; fontSize?: number                                  // 🆕 v0.5
...
export interface CanvasState {
  ...
  activePage: number
  selectedElementIds: string[]       // 🆕 v0.5
  editingElementId: string | null     // 🆕 v0.5
  clipboard: CanvasElement | null     // 🆕 v0.5

  updateElement: (page, id, attrs) => void  // 原子单元素更新
  addElement: (page, element) => void
  deleteElement: (page, id) => void
  setSlides: (slides) => void
  setActivePage: (page) => void
  // 🆕 v0.5 Actions
  setSelectedElements: (ids) => void
  setEditingElement: (id) => void
  duplicateSlide: (page) => void
  copyElement: (id) => void
  pasteElement: (page) => void
  moveLayer: (page, id, direction) => void
}

// zundo temporal middleware → 自动 Undo/Redo (limit: 50)
export const useCanvasStore = create<CanvasState>()(
  temporal(storeImpl, {
    limit: 50,
    partialize: (state) => ({ slides: state.slides, activePage: state.activePage }),
  })
)
```
...
### 10.11 数据转换层 (`lib/dataTransform.ts`) — v0.4 根治 + v0.5 扩展
...
const projectsApi = {
  ...
  uploadAsset: (projectId: string, file: File) => {
    ...
    return api.post(`/projects/${projectId}/assets`, fd).then(r => r.data);
  },
};
